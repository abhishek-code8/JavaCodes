...
public class DataMungerTestTask1 {
	...
	public void testGetSplitStrings()
	{
		...
		//System.out.println(dataMunger.getSplitStrings("select city,winner,player_match from ipl.csv where season > 2014 and city = 'Bangalore' order by city"));
	...
	public void testGetBaseQuery()
	{
		...
		//System.out.println(dataMunger.getBaseQuery("select city,winner,player_match from ipl.csv where season > 2014"));
	}
	...
	public void testGetBaseQueryFailure()
	{
		...
		assertNotEquals("testGetBaseQueryFailure(),Retrieval of Base Query failed. BaseQuery contains from the beginning of the query till the where clause",
				"select city,winner,player_match from ipl.csv", dataMunger.getBaseQuery("select city1,winner,player_match from ipl.csv where season > 2014"));
		//System.out.println(dataMunger.getBaseQuery("select city1,winner,player_match from ipl.csv where season > 2014"));
		assertNotNull("testGetBaseQueryFailure() , Retrieval of Base Query returns Null", dataMunger.getBaseQuery("select * from ipl.csv"));
		//System.out.println(dataMunger.getBaseQuery("select * from ipl.csv"));
	}
